...
gtf的第4、5列分别代表**某段被注释序列**在基因组上，起始和结束的坐标位点。
...
```Linux
#指令
-bash-4.1$ cat 1.gtf | awk '$3=="CDS" && $1=="XI"'|sort -k 5 -n |tail -10
...
```Linux
#指令
-bash-4.1$ cat 1.gtf |grep -v '#'| awk '$1=="IV"{print $3}'|sort|uniq -c|sort -k 1
